...
enum { base = 36, tmin = 1, tmax = 26, skew = 38, damp = 700, initial_bias = 72, initial_n = 0x80, delimiter = '-' }
;
...
@implementation NSString (TWTRPunycodeAdditions)
...
- (NSString *)punycodeEncodedString
{
    ...
    if (b > 0) [ret appendFormat:@"%C", (unichar)delimiter];
    ...
    while (h < input_length) {
        for (m = maxint, j = 0; j < input_length; ++j) {
            ...
            if (c >= n && c < m) m = longchars[j];
        }

        if (m - n > (maxint - delta) / (h + 1)) return nil;  // punycode_overflow;
        delta += (m - n) * (h + 1);
        n = m;
        ...
        for (j = 0; j < input_length; ++j) {
            ...
            if (c < n /* || basic([self characterAtIndex:j]) */) {
                if (++delta == 0) return nil;  // punycode_overflow;
            }
...
- (NSString *)punycodeDecodedString
{
    ...
    for (b = j = 0; j < input_length; ++j)
        if (delim([self characterAtIndex:j])) b = j;

    if (b > max_out) return nil;  // punycode_big_output;

    for (j = 0; j < b; ++j) {
        UTF32Char c = (UTF32Char)[self characterAtIndex:j];

        if (!basic([self characterAtIndex:j])) return nil;  // punycode_bad_input;
    ...
    for (NSUInteger inPos = b > 0 ? b + 1 : 0; inPos < input_length; ++outLen, ++i) {
        ...
        for (oldi = i, w = 1, k = base; /* nada */; k += base) {
            if (inPos >= input_length) return nil;  // punycode_bad_input;
            unsigned digit = (unsigned)decode_digit([self characterAtIndex:inPos++]);
            if (digit >= base) return nil;             // punycode_bad_input;
            if (digit > (maxint - i) / w) return nil;  // punycode_overflow;
            i += digit * w;
            t = k <= bias /* + tmin */ ? tmin : /* +tmin not needed */
                    k >= bias + tmax ? tmax : k - bias;
            if (digit < t) break;
            if (w > maxint / (base - t)) return nil;  // punycode_overflow;
            w *= (base - t);
        }

        bias = adapt((unsigned)i - (unsigned)oldi, (unsigned)outLen + 1, oldi == 0);

        if (i / (outLen + 1) > maxint - n) return nil;  // punycode_overflow;
        n += i / (outLen + 1);
        i %= (outLen + 1);
...
- (NSString *)IDNAEncodedString
{
    ...
    while (![s isAtEnd]) {
        ...
        if ([s scanCharactersFromSet:dotAt intoString:&input]) [ret appendString:input];
    }
    ...
    while (![s isAtEnd]) {
        if ([s scanUpToCharactersFromSet:dotAt intoString:&input]) {
            if ([input.lowercaseString hasPrefix:@"xn--"]) {
                ...
                if (substr) [ret appendString:substr];
            } else
                [ret appendString:input];
        }
...
- (NSDictionary *)URLParts
{
    ...
    if ([s scanUpToCharactersFromSet:colonSlash intoString:&host]) {
        if (!s.isAtEnd && [self characterAtIndex:s.scanLocation] == ':') {
            ...
            if (![s isAtEnd]) [s scanUpToString:@"/" intoString:&host];
        }
    }

    if (![s isAtEnd]) [s scanUpToString:@"#" intoString:&path];
    ...
    if ([s scanUpToCharactersFromSet:colonAt intoString:&temp]) {
        if (![s isAtEnd]) {
            ...
            if ([host characterAtIndex:[s scanLocation]] == ':') {
                ...
                if (![s isAtEnd] && [s scanUpToCharactersFromSet:colonAt intoString:&temp]) password = temp;
            }

            [s scanCharactersFromSet:colonAt intoString:nil];

            if (![s isAtEnd] && [s scanUpToCharactersFromSet:colonAt intoString:&temp]) host = temp;
        }
    }

    NSMutableDictionary *parts = [NSMutableDictionary dictionaryWithObjectsAndKeys:scheme, @"scheme", delim, @"delim", host, @"host", path, @"path", nil];

    if (username) parts[@"username"] = username;
    if (password) parts[@"password"] = password;
    if (fragment) parts[@"fragment"] = fragment;
...
- (NSString *)encodedURLString
{
    ...
    if (urlParts[@"username"]) {
        if (urlParts[@"password"]) {
            [ret appendFormat:@"%@:%@@", [urlParts[@"username"] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLUserAllowedCharacterSet]], [urlParts[@"password"] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLPasswordAllowedCharacterSet]]];
        } else
...
- (NSString *)decodedURLString
{
    ...
    if (username) {
        if (password)
            usernamePassword = [NSString stringWithFormat:@"%@:%@@", username.stringByRemovingPercentEncoding, password.stringByRemovingPercentEncoding];
        else
...
@implementation NSURL (TWTRPunycodeAdditions)
